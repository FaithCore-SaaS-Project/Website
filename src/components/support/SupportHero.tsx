    ...
    <section className="bg-gradient-to-br from-[#f7f5ff] to-white border-b py-20 lg:py-24">
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="mt-10 flex max-w-2xl overflow-hidden rounded-2xl border bg-white shadow-lg shadow-gray-200/60">
          ...
          <button
            aria-label="Search"
            className="flex items-center gap-2 bg-[#5B3DF5] px-8 font-semibold text-white hover:bg-[#4d31db] transition-colors text-sm"
          >
            <Search size={18} />
            Search
          </button>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          {["Add Members", "Events", "Donations", "Reports", "Billing"].map(
            (item) => (
              <button
                key={item}
                className="rounded-full border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-600 shadow-sm hover:border-[#5B3DF5]/40 hover:text-[#5B3DF5] transition-colors"
              >
                {item}
              </button>
            )
